...
def test_quant_engine_handles_insufficient_price_history_without_crash():
    ...
    trend_efficiency = result["metrics"]["algorithms"]["trend_efficiency_stability"]
    assert trend_efficiency["algorithm_id"] == "trend_efficiency_stability_v1"
    assert trend_efficiency["trend_efficiency_stability_score"] is None
    assert trend_efficiency["classification"] == "insufficient_data"
    assert trend_efficiency["used_in_composite_score"] is False
    market_resilience = result["metrics"]["algorithms"]["market_relative_resilience"]
    assert market_resilience["algorithm_id"] == "market_relative_resilience_v1"
    assert market_resilience["market_relative_resilience_score"] is None
    assert market_resilience["classification"] == "insufficient_data"
    assert market_resilience["used_in_composite_score"] is False
    tail_risk_momentum = result["metrics"]["algorithms"]["tail_risk_adjusted_momentum"]
    assert tail_risk_momentum["algorithm_id"] == "tail_risk_adjusted_momentum_v1"
    assert tail_risk_momentum["tail_risk_adjusted_momentum_score"] is None
    assert tail_risk_momentum["classification"] == "insufficient_data"
    assert tail_risk_momentum["used_in_composite_score"] is False
    accumulation_quality = result["metrics"]["algorithms"]["volume_accumulation_quality"]
    ...
    range_discipline = result["metrics"]["algorithms"]["range_discipline"]
    assert range_discipline["algorithm_id"] == "range_discipline_v1"
    assert range_discipline["range_discipline_score"] is None
    assert range_discipline["classification"] == "insufficient_data"
    assert range_discipline["used_in_composite_score"] is False
    volatility_compression = result["metrics"]["algorithms"]["volatility_compression_readiness"]
    assert volatility_compression["algorithm_id"] == "volatility_compression_readiness_v1"
    assert volatility_compression["volatility_compression_readiness_score"] is None
    assert volatility_compression["classification"] == "insufficient_data"
    assert volatility_compression["used_in_composite_score"] is False
...
def test_factor_risk_hybrid_and_signal_are_deterministic():
    ...
    assert quant["component_scores"]["trend_efficiency_stability"] is not None
    ...
    assert quant["component_scores"]["range_discipline"] is not None
    assert quant["component_scores"]["volatility_compression_readiness"] is not None
    ...
    assert liquidity_stability["used_in_composite_score"] is False
    trend_efficiency = quant["metrics"]["algorithms"]["trend_efficiency_stability"]
    assert trend_efficiency["algorithm_id"] == "trend_efficiency_stability_v1"
    assert trend_efficiency["not_investment_advice"] is True
    assert trend_efficiency["used_in_composite_score"] is False
    market_resilience = quant["metrics"]["algorithms"]["market_relative_resilience"]
    assert market_resilience["algorithm_id"] == "market_relative_resilience_v1"
    assert market_resilience["not_investment_advice"] is True
    assert market_resilience["used_in_composite_score"] is False
    tail_risk_momentum = quant["metrics"]["algorithms"]["tail_risk_adjusted_momentum"]
    assert tail_risk_momentum["algorithm_id"] == "tail_risk_adjusted_momentum_v1"
    assert tail_risk_momentum["not_investment_advice"] is True
    assert tail_risk_momentum["used_in_composite_score"] is False
    accumulation_quality = quant["metrics"]["algorithms"]["volume_accumulation_quality"]
    assert accumulation_quality["algorithm_id"] == "volume_accumulation_quality_v1"
    ...
    assert gap_risk_stability["used_in_composite_score"] is False
    range_discipline = quant["metrics"]["algorithms"]["range_discipline"]
    assert range_discipline["algorithm_id"] == "range_discipline_v1"
    assert range_discipline["not_investment_advice"] is True
    assert range_discipline["used_in_composite_score"] is False
    volatility_compression = quant["metrics"]["algorithms"]["volatility_compression_readiness"]
    assert volatility_compression["algorithm_id"] == "volatility_compression_readiness_v1"
    assert volatility_compression["not_investment_advice"] is True
    assert volatility_compression["used_in_composite_score"] is False
...
def test_ai_and_qa_interpret_without_overriding_signal_or_giving_orders():
    ...
    assert context["quant_snapshot"]["trend_efficiency_stability"]["algorithm_id"] == "trend_efficiency_stability_v1"
    ...
    assert context["quant_snapshot"]["range_discipline"]["algorithm_id"] == "range_discipline_v1"
    assert context["quant_snapshot"]["volatility_compression_readiness"]["algorithm_id"] == "volatility_compression_readiness_v1"
    ...
    assert "trend_efficiency_stability_v1" in str(report["report"]["key_changes"])
    ...
    assert "range_discipline_v1" in str(report["report"]["key_changes"])
    assert "volatility_compression_readiness_v1" in str(report["report"]["key_changes"])
