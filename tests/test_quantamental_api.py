...
def test_quantamental_analysis_endpoint_shape(monkeypatch):
    ...
    assert body["quant"]["metrics"]["algorithms"]["tail_risk_adjusted_momentum"]["algorithm_id"] == "tail_risk_adjusted_momentum_v1"
    assert body["quant"]["metrics"]["algorithms"]["tail_risk_adjusted_momentum"]["used_in_composite_score"] is False
    assert body["quant"]["metrics"]["algorithms"]["volume_accumulation_quality"]["algorithm_id"] == "volume_accumulation_quality_v1"
    assert body["quant"]["metrics"]["algorithms"]["volume_accumulation_quality"]["used_in_composite_score"] is False
    assert body["quant"]["metrics"]["algorithms"]["gap_risk_stability"]["algorithm_id"] == "gap_risk_stability_v1"
    assert body["quant"]["metrics"]["algorithms"]["gap_risk_stability"]["used_in_composite_score"] is False
    assert body["quant"]["metrics"]["algorithms"]["range_discipline"]["algorithm_id"] == "range_discipline_v1"
    assert body["quant"]["metrics"]["algorithms"]["range_discipline"]["used_in_composite_score"] is False
    assert body["quant"]["metrics"]["algorithms"]["volatility_compression_readiness"]["algorithm_id"] == "volatility_compression_readiness_v1"
    assert body["quant"]["metrics"]["algorithms"]["volatility_compression_readiness"]["used_in_composite_score"] is False
...
def test_quantamental_score_screen_supports_trend_efficiency_score(monkeypatch):
    quantamental_cache.clear()
    trend_scores = {"AAA": 84.0, "BBB": 56.0, "CCC": 78.0}

    def fake_analysis(request):
        score = trend_scores[request.ticker]
        return {
            "status": "ok",
            "ticker": request.ticker,
            "market": request.market,
    ...
    resp = client.get(
        "/api/v1/quantamental/screen/by-score"
        "?tickers=AAA%20BBB%20CCC&score_key=trend_efficiency&min_score=70&limit=10&include_ai=false"
    )

    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] == "ok"
    assert body["score_key"] == "trend_efficiency"
    assert body["score_label"] == "Trend Efficiency"
    assert [row["ticker"] for row in body["matches"]] == ["AAA", "CCC"]
    assert all(row["screen_score_key"] == "trend_efficiency" for row in body["matches"])
...
def test_quantamental_score_screen_supports_tail_risk_momentum_score(monkeypatch):
    quantamental_cache.clear()
    tail_risk_scores = {"AAA": 82.0, "BBB": 58.0, "CCC": 76.0}

    def fake_analysis(request):
        score = tail_risk_scores[request.ticker]
        return {
            "status": "ok",
            "ticker": request.ticker,
            "market": request.market,
    ...
    resp = client.get(
        "/api/v1/quantamental/screen/by-score"
        "?tickers=AAA%20BBB%20CCC&score_key=tail_risk_momentum&min_score=70&limit=10&include_ai=false"
    )

    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] == "ok"
    assert body["score_key"] == "tail_risk_momentum"
    assert body["score_label"] == "Tail Risk Momentum"
    assert [row["ticker"] for row in body["matches"]] == ["AAA", "CCC"]
    assert all(row["screen_score_key"] == "tail_risk_momentum" for row in body["matches"])
    assert all(row["tail_risk_momentum_score"] >= 70 for row in body["matches"])


def test_quantamental_score_screen_supports_accumulation_quality_score(monkeypatch):
    quantamental_cache.clear()
    accumulation_scores = {"AAA": 83.0, "BBB": 59.0, "CCC": 77.0}

    def fake_analysis(request):
        score = accumulation_scores[request.ticker]
        return {
            "status": "ok",
            "ticker": request.ticker,
            "market": request.market,
    ...
    resp = client.get(
        "/api/v1/quantamental/screen/by-score"
        "?tickers=AAA%20BBB%20CCC&score_key=accumulation_quality&min_score=70&limit=10&include_ai=false"
    )

    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] == "ok"
    assert body["score_key"] == "accumulation_quality"
    assert body["score_label"] == "Accumulation Quality"
    assert [row["ticker"] for row in body["matches"]] == ["AAA", "CCC"]
    assert all(row["screen_score_key"] == "accumulation_quality" for row in body["matches"])
    assert all(row["accumulation_quality_score"] >= 70 for row in body["matches"])


def test_quantamental_score_screen_supports_gap_risk_stability_score(monkeypatch):
    quantamental_cache.clear()
    gap_scores = {"AAA": 84.0, "BBB": 57.0, "CCC": 78.0}

    def fake_analysis(request):
        score = gap_scores[request.ticker]
        return {
            "status": "ok",
            "ticker": request.ticker,
            "market": request.market,
    ...
    resp = client.get(
        "/api/v1/quantamental/screen/by-score"
        "?tickers=AAA%20BBB%20CCC&score_key=gap_risk_stability&min_score=70&limit=10&include_ai=false"
    )

    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] == "ok"
    assert body["score_key"] == "gap_risk_stability"
    assert body["score_label"] == "Gap Risk Stability"
    assert [row["ticker"] for row in body["matches"]] == ["AAA", "CCC"]
    assert all(row["screen_score_key"] == "gap_risk_stability" for row in body["matches"])
    assert all(row["gap_risk_stability_score"] >= 70 for row in body["matches"])


def test_quantamental_score_screen_supports_range_discipline_score(monkeypatch):
    quantamental_cache.clear()
    range_scores = {"AAA": 83.0, "BBB": 56.0, "CCC": 77.0}

    def fake_analysis(request):
        score = range_scores[request.ticker]
        return {
            "status": "ok",
            "ticker": request.ticker,
            "market": request.market,
    ...
    resp = client.get(
        "/api/v1/quantamental/screen/by-score"
        "?tickers=AAA%20BBB%20CCC&score_key=range_discipline&min_score=70&limit=10&include_ai=false"
    )

    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] == "ok"
    assert body["score_key"] == "range_discipline"
    assert body["score_label"] == "Range Discipline"
    assert [row["ticker"] for row in body["matches"]] == ["AAA", "CCC"]
    assert all(row["screen_score_key"] == "range_discipline" for row in body["matches"])
    assert all(row["range_discipline_score"] >= 70 for row in body["matches"])


def test_quantamental_score_screen_supports_volatility_compression_score(monkeypatch):
    quantamental_cache.clear()
    compression_scores = {"AAA": 84.0, "BBB": 55.0, "CCC": 78.0}

    def fake_analysis(request):
        score = compression_scores[request.ticker]
        return {
            "status": "ok",
            "ticker": request.ticker,
            "market": request.market,
    ...
    resp = client.get(
        "/api/v1/quantamental/screen/by-score"
        "?tickers=AAA%20BBB%20CCC&score_key=volatility_compression&min_score=70&limit=10&include_ai=false"
    )

    assert resp.status_code == 200
    body = resp.json()
    assert body["status"] == "ok"
    assert body["score_key"] == "volatility_compression"
    assert body["score_label"] == "Volatility Compression"
    assert [row["ticker"] for row in body["matches"]] == ["AAA", "CCC"]
    assert all(row["screen_score_key"] == "volatility_compression" for row in body["matches"])
    assert all(row["volatility_compression_score"] >= 70 for row in body["matches"])
...
def test_quantamental_health_lists_global_as_supported_and_quant_uses_global_benchmark(monkeypatch):
    ...
    assert "tail_risk_adjusted_momentum_v1" in health["enhancements"]
    assert "volume_accumulation_quality_v1" in health["enhancements"]
    assert "gap_risk_stability_v1" in health["enhancements"]
    assert "range_discipline_v1" in health["enhancements"]
    assert "volatility_compression_readiness_v1" in health["enhancements"]
    assert any(item["key"] == "market_resilience" for item in health["score_screen_keys"])
    assert any(item["key"] == "tail_risk_momentum" for item in health["score_screen_keys"])
    assert any(item["key"] == "accumulation_quality" for item in health["score_screen_keys"])
    assert any(item["key"] == "gap_risk_stability" for item in health["score_screen_keys"])
    assert any(item["key"] == "range_discipline" for item in health["score_screen_keys"])
    assert any(item["key"] == "volatility_compression" for item in health["score_screen_keys"])
